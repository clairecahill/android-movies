...
public class MovieRepository {
    ...
    public MovieRepository() {
        ...
        Gson gson = new GsonBuilder()
                .registerTypeAdapter(PopularMovieData.class, new PopularMovieData.JsonDeserializer())
                .create();
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(BASE_URL)
                .addConverterFactory(GsonConverterFactory.create(gson))
                .build();
        this.popularMovieService = retrofit.create(PopularMovieService.class);
    }
...
//        if (shouldFetchForecast(location, units)) {
            ...
            Call<PopularMovies> req = this.popularMovieService.fetchPopularMovies(apiKey);
            req.enqueue(new Callback<PopularMovies>() {
                ...
                public void onResponse(Call<PopularMovies> call, Response<PopularMovies> response) {
                    if (response.code() == 200) {
                        ...
                        Log.d(TAG, response.body().getPopularMovieResults().get(0).getTitle());
